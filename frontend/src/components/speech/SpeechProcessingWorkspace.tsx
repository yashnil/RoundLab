...
import { Check, Pencil, Copy, Swords, RefreshCw } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { CardContent } from "@/components/ui/card";
import { fadeUp } from "@/lib/motion";
import {
  StepHeader, InlineAlert, WorkspaceCard, FlowSummary, FlowLensNote, ContextualHelp,
  FlowCoachNote, TopIssueCoachNote, CoachDiagnosis, isReportStale,
...
import type {
  Speech, Transcript, AnalysisJob, ArgumentMap, ArgumentItem, FeedbackReport, Drill,
  DrillStatus, Workout, DeliveryMetrics, BlockCoverageResponse,
} from "@/types";
...
const STAGE_MESSAGES: Record<"transcript" | "flow" | "feedback", { title: string; messages: string[] }> = {
  transcript: { title: "Preparing your speech", messages: ["Transcribing audio", "Processing speech text", "Analyzing word choice"] },
  flow: { title: "Mapping arguments", messages: ["Identifying claims", "Extracting warrants and evidence", "Building your flow table"] },
  feedback: { title: "Applying rubric", messages: ["Evaluating structure and warranting", "Scoring each dimension", "Building coaching report"] },
};

export interface SpeechProcessingWorkspaceProps {
  speech: Speech;
  transcript: Transcript | null;
  analyzingUnified: boolean;
  activeJob: AnalysisJob | null;
  canAnalyze: boolean;
  startJobAnalysis: () => void;
  unifiedAnalysisErr: string;
  retryAnalysis: () => void;
  retryingJob: boolean;
  analysisStage: "transcript" | "flow" | "feedback" | null;
...
export default function SpeechProcessingWorkspace({
  speech, transcript, analyzingUnified, activeJob, canAnalyze, startJobAnalysis,
  ...
  drillErr, generateDrills,
}: SpeechProcessingWorkspaceProps) {
  return (
              <>
