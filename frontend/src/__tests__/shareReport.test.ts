...
describe("formatPracticePlan", () => {
  it("starts with RoundLab Practice Plan header", () => {
    const text = formatPracticePlan(makeFull());
    expect(text.startsWith("RoundLab Practice Plan")).toBe(true);
  });
  ...
  it("handles null feedback without crashing", () => {
    ...
    expect(text).toContain("RoundLab Practice Plan");
  });
  ...
  it("ends with RoundLab attribution", () => {
    expect(formatPracticePlan(makeFull())).toContain("Generated by RoundLab");
  });
